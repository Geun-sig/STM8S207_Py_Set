...
## 최근 업데이트 (2026-01-09) - 장비 설정 UI 개선

### 장비 설정 다이얼로그 UI 개선 (`src/gui/widgets/device_settings_dialog.py`)

#### 1. 그룹박스 제목 폰트 스타일링 개선

**문제:**
- "배터리 타입 선택" 및 "충방전 설정" QGroupBox 제목이 시각적으로 눈에 잘 띄지 않음.
- `setStyleSheet`를 이용한 폰트 크기 변경이 일부 환경에서 적용되지 않는 문제 발생.

**해결:**
- **`QFont` 객체 사용:** 스타일시트의 불안정성을 해결하기 위해, `QFont` 객체를 직접 생성하여 폰트 크기와 굵기를 설정하는 방식으로 변경. 이 방법은 스타일시트보다 우선순위가 높아 더 안정적으로 스타일을 적용할 수 있음.
- **스타일 분리:** `setFont`로 폰트의 크기(12pt)와 굵기(`Bold`)를 설정하고, `setStyleSheet`는 텍스트 색상(`#0000A0`)만 담당하도록 역할을 분리함.
- **일관성 적용:** "배터리 타입 선택"과 "충방전 설정" 두 그룹박스 제목에 동일한 스타일을 적용하여 일관성을 확보.
...
#### 2. 레이블 텍스트 변경
- **목적:** 사용자 이해도 향상
- **변경 내용:** "현재 사용 배터리 타입:" 레이블을 "설정 배터리 타입:"으로 변경함.

**수정된 코드:**
```python
# 기존
# type_layout.addRow("현재 사용 배터리 타입:", self.battery_type_combo)
...
#### 3. 버그 수정
- **문제:** `QFont`를 적용하는 과정에서 실수로 `PyQt6.QtWidgets`의 import 구문이 삭제되어 `NameError: name 'QDialog' is not defined` 오류 발생.
- **해결:** 누락된 `QtWidgets` import 구문을 `device_settings_dialog.py` 파일 상단에 다시 추가하여 오류 해결.
